...
memento.updateLastMod = function (date) {
    $("#lastMod").attr("data-timestamp", new Date(date));
    $("#lastMod").attr("title", memento.formatDate(date, true));
    $("#lastMod").cuteTime({ refresh: 10000 })
};

memento.clearLastMod = function() {
    $("#lastMod").cuteTime.stop_cuteness();
    $("#lastMod").html("");
};
...
memento.changeScreen = function (target) {
    ...
    $("#" + target).show();
    memento.resizeMe(target == "openNote");
};
...
memento.resizeMe = function(resizeEditor){
    ...
    if(resizeEditor) {
        ...
        editorHeight -= $("#openNote").height() - $("#content").height();
...
memento.setNoteContent = function (docId, content) {
    ...
    gdocs.getDocById(docId, function(doc){
        bgPage.doc = doc;
        $("#openNote").attr('etag', doc.entry.gd$etag);
        $("#openNote #inputTitle").val(doc.title);
...
memento.manageNoteChange = function (e) {
    ...
    if(((bgPage.doc && bgPage.doc.resourceId) && !bgPage.isUpdating) && jQuery.inArray(e.keyCode, keyCodes) == '-1'){
        ...
        typingTimer = setTimeout(function(){
            if($("#inputTitle").val() && $("#inputTitle").val().length > 0) {
                bgPage.isUpdating = true;

                $("#inputTitle").removeClass("error");
                /*gdocs.getDocById(bgPage.doc.resourceId, function(doc){
                    if(doc.entry.gd$etag != $("#openNote").attr('etag')) {
...
$(document).ready(function () {
        ...
        var getFolderIdByTitleCallback = function(folderId) {
            ...
            if(localStorage.getItem("singleNote") !== null && localStorage["singleNote"] == 'true'){
                ...
                if(bgPage.doc && bgPage.doc.resourceId) {
                    ...
                    memento.changeScreen('mainScreen');
    ...
    $('#inputTitle').change(function(){
        if($("#inputTitle").val().length == 0) {
            $("#inputTitle").addClass("error");
        } else if($("#inputTitle").hasClass("error")) {
            $("#inputTitle").removeClass("error");
        }
    });

    $("#inputTitle").attr("placeholder", chrome.i18n.getMessage("title"));
        ...
        for(var i = 0; i < bgPage.docs.length; i++ ) {
            ...
            if($('#inputSearch').val()) {
                if (doc.title.toLowerCase().indexOf($('#inputSearch').val().toLowerCase()) == -1){
                    $(".noteListItem." + doc.resourceId).hide();
                } else {
                    $(".noteListItem." + doc.resourceId).show();
                }
            } else {
                $(".noteListItem." + doc.resourceId).show();
            }
...
//    });
    ...
    // misc
    $('#noteList').on('click', '.noteListItem .docTitle', function(){
