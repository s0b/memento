	...
	$.fn.cuteTime.settings = {
		...
		time_ranges: [
			{bound: NEG_INF,			// IMPORANT: bounds MUST be in ascending order, from negative infinity to positive infinity
					cuteness: "just_now",		    unit_size: 0},
			{bound: 0, 
					cuteness: "just_now",			unit_size: 0},
			{bound: 20 * 1000, 
					cuteness: "seconds_ago",	    unit_size: 0},
			{bound: 60 * 1000, 
					cuteness: "1_minute_ago",		unit_size: 0},
			{bound: 60 * 1000 * 2, 
					cuteness: "x_minutes_ago",		unit_size: 60 * 1000},
			{bound: 60 * 1000 * 60, 
					cuteness: "1_hour_ago",		    unit_size: 0},
			{bound: 60 * 1000 * 60 * 2, 
					cuteness: "x_hours_ago",		unit_size: 60 * 1000 * 60},
			{bound: 60 * 1000 * 60 * 24, 
					cuteness: "1_day_ago",			unit_size: 0},
			{bound: 60 * 1000 * 60 * 24 * 2, 
					cuteness: "x_days_ago",			unit_size: 60 * 1000 * 60 * 24},
			{bound: 60 * 1000 * 60 * 24 * 30,	
					cuteness: "1_month_ago",		unit_size: 0},
			{bound: 60 * 1000 * 60 * 24 * 30 * 2, 
					cuteness: "x_months_ago",		unit_size: 60 * 1000 * 60 * 24 * 30},
			{bound: 60 * 1000 * 60 * 24 * 30 * 12, 
					cuteness: "1_year_ago",			unit_size: 0},
			{bound: 60 * 1000 * 60 * 24 * 30 * 12 * 2, 
					cuteness: "x_years_ago",		unit_size: 60 * 1000 * 60 * 24 * 30 * 12},
			{bound: POS_INF, 
					cuteness: "a_while_ago",		unit_size: 0}
		]
	};
	...
	**********************************************************************************/
	$.fn.cuteTime.start_cuteness = function() {
		...
		if ($.fn.cuteTime.process_tracker == null) {
			if (refresh_rate > 0) {
				$.fn.cuteTime.process_tracker = setInterval(function() {
                    $.fn.cuteTime.update_cuteness();
                }, refresh_rate );
			}
		} else { 
		...
		jQuery.each(time_ranges, function(i, timespan) {
			if (i < time_ranges.length-1) {
				if ((	time_difference		>=		timespan['bound']) &&
                    ...
                    cute_time = chrome.i18n.getMessage(timespan['cuteness']).replace(/%CT%/, calculated_time);
